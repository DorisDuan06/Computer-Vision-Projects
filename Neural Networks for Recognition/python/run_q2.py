...
for k, v in params.items():
    ...
    if 'W' in k:
        ...
        for i in range(N):
            for j in range(D):
                ...
                #   add epsilon
                params_add[k][i, j] += eps
                params_minus[k][i, j] -= eps
                #   run the network
                h1_add = forward(x, params_add, 'layer1')
                probs_add = forward(h1_add, params_add, 'output', softmax)
                h1_minus = forward(x, params_minus, 'layer1')
                probs_minus = forward(h1_minus, params_minus, 'output', softmax)
                #   get the loss
                loss_add, _ = compute_loss_and_acc(y, probs_add)
                loss_minus, _ = compute_loss_and_acc(y, probs_minus)
                #   compute derivative with central diffs
                params['grad_'+k][i, j] = (loss_add - loss_minus) / (2 * eps)
    ...
    if 'b' in k:
        ...
        for i in range(1):
            ...
            loss_minus, _ = compute_loss_and_acc(y, probs_minus)
            #   compute derivative with central diffs
            params['grad_'+k][i] = (loss_add - loss_minus) / (2 * eps)
